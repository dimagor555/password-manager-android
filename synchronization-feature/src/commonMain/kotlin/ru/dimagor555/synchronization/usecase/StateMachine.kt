...
interface IStateMachine<STATE : Any, EVENT : Any, SIDE_EFFECT : Any> {
    val state: STATE
...
class StateMachine<STATE : Any, EVENT : Any, SIDE_EFFECT : Any> private constructor(
...
) : IStateMachine<STATE, EVENT, SIDE_EFFECT> {
    ...
    override fun transition(event: EVENT): Transition<STATE, EVENT, SIDE_EFFECT> {
...
data class StateMachineInvalidTransitionException(
    val transition: StateMachine.Transition<*, *, *>
) : IllegalStateException() {
    override val message = transition.toString()
}